...
import { ROLE_ADMIN_MUTATION_PRESENTER_KEY } from "@modules/role/shared/role.presenter-keys";
...
const CREATE_ROLE_FROM_TEMPLATE_BODY_FIELDS: readonly string[] = Object.freeze([
  "templateCode",
  "name",
  "code",
  "description",
...
const UPDATE_ROLE_BODY_FIELDS: readonly string[] = Object.freeze([
  "name",
  "description",
  "delegationBand",
  "maxDelegatableBand",
...
const SET_ROLE_PERMISSIONS_BODY_FIELDS: readonly string[] = Object.freeze([
  "permissions",
]);

const SET_ROLE_ASSIGNMENT_RULES_BODY_FIELDS: readonly string[] = Object.freeze([
  "rules",
]);

const ROLE_ASSIGNMENT_RULE_FIELDS: readonly string[] = Object.freeze([
  "id",
  "code",
  "description",
  "state",
...
export class AdminRoleController extends SecureController {
  ...
  protected async handle(
    ...
    const command = readCommand<RoleMutationCommand>(req);
    ...
    switch (command) {
      case "ROLE_CREATE":
        return this.service.createRole(actor, parseCreateRoleCommand(req));

      case "ROLE_CREATE_FROM_TEMPLATE":
        return this.service.createRoleFromTemplate(
          actor,
          parseCreateRoleFromTemplateCommand(req),
        );

      case "ROLE_UPDATE":
        return this.service.updateRole(actor, parseUpdateRoleCommand(req));

      case "ROLE_ACTIVATE":
        return this.service.activateRole(actor, parseActivateRoleCommand(req));
      ...
      case "ROLE_ARCHIVE":
        return this.service.archiveRole(actor, parseArchiveRoleCommand(req));
  ...
  protected async present(
    ...
    return getPresenterRegistryFromRequest(req)
      .get<unknown, PresentationResult>(ROLE_ADMIN_MUTATION_PRESENTER_KEY)
      .present(result, context);
  }
}

function parseCreateRoleCommand(req: Request): CreateRoleCommand {
  const body = requireRecord(req.body);
  assertNoUnexpectedFields(body, CREATE_ROLE_BODY_FIELDS, "ROLE_CREATE");
  ...
  return {
    ...
    initialPermissions: body.initialPermissions as
      | readonly string[]
      | undefined,
    initialDelegationBand: body.initialDelegationBand as
      | CreateRoleCommand["initialDelegationBand"]
      | undefined,
    initialMaxDelegatableBand: body.initialMaxDelegatableBand as
      | CreateRoleCommand["initialMaxDelegatableBand"]
      | undefined,
    initialAssignmentRules: body.initialAssignmentRules as
      | SetRoleAssignmentRulesCommand["rules"]
      | undefined,
  };
}

function parseUpdateRoleCommand(req: Request): UpdateRoleCommand {
  const body = requireRecord(req.body);
  assertNoUnexpectedFields(body, UPDATE_ROLE_BODY_FIELDS, "ROLE_UPDATE");

  return {
    roleId: req.params.roleId,
    name: body.name as string | null | undefined,
    description: body.description as string | null | undefined,
    delegationBand: body.delegationBand as
      | UpdateRoleCommand["delegationBand"]
      | undefined,
    maxDelegatableBand: body.maxDelegatableBand as
      | UpdateRoleCommand["maxDelegatableBand"]
      | undefined,
  };
}

function parseActivateRoleCommand(req: Request): ActivateRoleCommand {
  assertNoUnexpectedFields(
    requirePlainObjectBodyForZeroBodyMutation(req.body, "ROLE_ACTIVATE"),
    [],
    "ROLE_ACTIVATE",
  );
...
function parseCreateRoleFromTemplateCommand(
  req: Request,
): CreateRoleFromTemplateCommand {
  const body = requireRecord(req.body);
  assertNoUnexpectedFields(
    body,
    CREATE_ROLE_FROM_TEMPLATE_BODY_FIELDS,
    "ROLE_CREATE_FROM_TEMPLATE",
  );
...
function parseDeactivateRoleCommand(req: Request): DeactivateRoleCommand {
...
function parseArchiveRoleCommand(req: Request): ArchiveRoleCommand {
  const body = requirePlainObjectBodyForOptionalReasonMutation(
    req.body,
    "ROLE_ARCHIVE",
  );
  assertNoUnexpectedFields(body, OPTIONAL_REASON_BODY_FIELDS, "ROLE_ARCHIVE");
...
function parseSetRolePermissionsCommand(
  ...
  return {
    roleId: req.params.roleId,
    permissions: body.permissions as readonly string[],
  };
}
...
function requireRequiredArrayFieldBody(
  ...
  const body = requirePlainObjectBody(value, command);
  assertNoUnexpectedFields(body, allowedFields, command);
  ...
  if (!Array.isArray(body[requiredField])) {
    throw new RoleValidationError(`${requiredField} must be an array`);
  }
...
function assertNoUnexpectedAssignmentRuleFields(
  ...
  for (let index = 0; index < rules.length; index += 1) {
    ...
    const unexpectedFields = Object.keys(rule).filter(
      (field) => !ROLE_ASSIGNMENT_RULE_FIELDS.includes(field),
    );
